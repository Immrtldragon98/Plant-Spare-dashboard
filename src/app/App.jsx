...
export default function App(){
 ...
 const reload=async()=>{if(!user)return;const qs=new URLSearchParams({...filters,search});const [s,o]=await Promise.all([request('/dashboard?'+qs),request('/options?'+qs)]);setStats(s);setOptions(o);if(tab==='Equipment')setMaterials(await request('/materials?'+qs));if(tab==='Vendors')setVendors(await request('/vendors?'+qs));if(tab==='Imports'&&user.role!=='viewer')setHistory(await request('/import-history'));if(tab==='Users'&&isAdmin(user.role))setUsers(await request('/users'));if(tab==='SAP Hierarchy'&&isAdmin(user.role))setHierarchy(await request('/hierarchy?'+new URLSearchParams({department_code:filters.department_code})));setRefreshToken(x=>x+1)};
 ...
 const canEdit=canEditRole(user.role),admin=isAdmin(user.role);const tabs=['Dashboard','Spares','Equipment','Vendors','Procurement','Spare Intelligence','Knowledge',...(canEdit?['Imports']:[]),...(admin?['Departments','SAP Hierarchy','Users']:[])];
 const selectedDepartment=options.departments?.find(d=>d.department_code===filters.department_code);
 return <div><header><div><strong>SPARE MATERIALS</strong><span>{selectedDepartment?.department_code||filters.department_code} · {selectedDepartment?.department_name||''}</span></div><div className="user">{user.name}<small>{user.role}</small><button className="ghost" onClick={()=>{localStorage.removeItem('token');setUser(null)}}>Logout</button></div></header><nav>{tabs.map(x=><button className={tab===x?'active':''} onClick={()=>setTab(x)} key={x}>{x}</button>)}</nav><main>{notice&&<div className="notice" onClick={()=>setNotice('')}>{notice}</div>}{tab==='Dashboard'&&<Dashboard stats={stats} setTab={setTab} department={selectedDepartment} filters={filters} setFilters={setFilters} options={options}/>} {tab==='Spares'&&<Spares {...{options,search,setSearch,filters,setFilters,canEdit,setEditing,reload,setNotice,refreshToken}}/>}{tab==='Equipment'&&<Equipment {...{materials,options,filters,setFilters,setTab}}/>} {tab==='Vendors'&&<Vendors vendors={vendors} setFilters={setFilters} setTab={setTab}/>} {tab==='Procurement'&&<Procurement filters={filters} setFilters={setFilters} setNotice={setNotice}/>} {tab==='Spare Intelligence'&&<SpareIntelligence filters={filters} options={options} setNotice={setNotice}/>} {tab==='Knowledge'&&<Knowledge filters={filters} options={options} canEdit={canEdit} setNotice={setNotice}/>} {tab==='Imports'&&canEdit&&<Imports history={history} filters={filters} options={options} reload={reload} setNotice={setNotice}/>} {tab==='Departments'&&admin&&<Departments departments={options.departments} reload={reload} setNotice={setNotice}/>} {tab==='SAP Hierarchy'&&admin&&<Hierarchy rows={hierarchy} departments={options.departments} reload={reload} setNotice={setNotice}/>} {tab==='Users'&&admin&&<Users users={users} reload={reload} setNotice={setNotice}/>}</main><SpareCopilot departmentCode={filters.department_code} tab={tab} filters={filters} setNotice={setNotice}/>{editing&&<MaterialModal material={editing===true?{department_code:filters.department_code,area:filters.area,discipline:filters.discipline}:editing} departments={options.departments} onClose={()=>setEditing(null)} onSaved={()=>{setEditing(null);reload()}} setNotice={setNotice}/>}</div>
}
