...
r.get('/options',auth,async(req,res)=>{
  ...
  let areas=[];
  if(department_code){areas=(await q(`SELECT a.area_name FROM areas a JOIN departments d ON d.id=a.department_id WHERE a.active=true AND d.department_code=$1 ORDER BY a.area_name`,[department_code])).rows.map(x=>x.area_name)}else areas=[...new Set(rows.map(x=>x.area_name).filter(Boolean))].sort();
