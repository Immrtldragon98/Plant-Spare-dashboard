...
export default function Hierarchy({rows,departments,reload,setNotice}){
  ...
  return <>
    <div className="pageTitle"><div><h1>SAP Hierarchy</h1><p>Plant → Department → Area → Equipment Code → Sub-equipment. Discipline remains a separate filter.</p></div><button onClick={()=>setEdit({department_code:departments?.[0]?.department_code||''})}>+ Add Location</button></div>
    <div className="tableWrap"><table><thead><tr><th>Plant</th><th>Department</th><th>Area</th><th>Equipment Code</th><th>Sub-equipment</th><th>Sub-equipment Code</th><th>Status</th><th></th></tr></thead><tbody>{rows.map(r=><tr key={r.id}><td className="code"><Blank value={r.plant_code||'3102'}/></td><td><div>{r.department_name}</div><small className="muted">{r.department_code}</small></td><td>{r.area_name}</td><td className="code"><Blank value={r.equipment_code}/></td><td><Blank value={r.sub_equipment_name}/></td><td className="code"><Blank value={r.sub_equipment_code}/></td><td><span className={(r.area_code||r.equipment_code||r.sub_equipment_code)?'mapped':'unmapped'}>{(r.area_code||r.equipment_code||r.sub_equipment_code)?'Mapped':'Needs mapping'}</span></td><td><button className="link" onClick={()=>setEdit(r)}>Edit</button></td></tr>)}</tbody></table></div>
...
function HierarchyModal({item,departments,onClose,onSaved,setNotice}){
  ...
  return <div className="modal"><form onSubmit={async e=>{e.preventDefault();try{const payload={...x,equipment_name:null,sap_location_code:null};await request(item.id?`/hierarchy/${item.id}`:'/hierarchy',{method:item.id?'PUT':'POST',body:JSON.stringify(payload)});onSaved()}catch(e){setNotice(e.message)}}}>
    ...
    <div className="formGrid">
      ...
      {field('area_code','SAP Area Code','3102_CH2_WRM')}
      {field('equipment_code','Equipment Code','SAP equipment / technical-object code')}
