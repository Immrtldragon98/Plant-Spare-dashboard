...
function canonicalRow(row,headers,mapping,sheetName,rowNumber,fileType,defaultDiscipline=''){
  ...
  const out={
    ...
    spare_name:descriptive(read(row,headers,mapping,'spare_name')),
    description:descriptive(read(row,headers,mapping,'description')),
    ...
    manufacturer:descriptive(read(row,headers,mapping,'manufacturer')),
    assembly_name:descriptive(read(row,headers,mapping,'assembly_name')),
    equipment_description:descriptive(read(row,headers,mapping,'equipment_description')),
    ...
    po_item:clean(read(row,headers,mapping,'po_item'))||null,
    ...
    justification:descriptive(read(row,headers,mapping,'justification')),
    notes:descriptive(read(row,headers,mapping,'notes')),
    failure_root_cause:descriptive(read(row,headers,mapping,'failure_root_cause')),
  ...
  if(fileType==='open_po'){
    out.store_qty=null;out.pr_qty=null;out.required_qty=null;out.planned_pr_qty=null;out.safety_stock=null;out.ideal_pr_qty=null;
  }else if(fileType==='open_pr'){
    out.store_qty=null;out.po_qty=null;out.required_qty=null;out.safety_stock=null;out.ideal_pr_qty=null;
  }else if(fileType==='stock'){
    out.pr_qty=null;out.po_qty=null;out.required_qty=null;out.planned_pr_qty=null;out.safety_stock=null;out.ideal_pr_qty=null;
  }
...
function localSheetMapping(sheetSummary){
  ...
  const aliases={
    ...
    po_number:['purchasing document','po number','purchase order','po no'],po_item:['purchasing document item','purchase order item','po item'],pr_number:['purchase requisition','pr number','pr no'],pr_item:['requisn item','requisition item','pr item']
  };
