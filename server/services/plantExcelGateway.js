...
export function snapshotRowsFromCanonical(fileType,rows=[]){
  if(fileType==='open_po'||fileType==='open_pr'){
    ...
    for(const r of rows){
      ...
      const qty=Number(fileType==='open_po'?r.po_qty:r.pr_qty);if(!Number.isFinite(qty))continue;
      const doc=clean(fileType==='open_po'?r.po_number:r.pr_number);
      const item=clean(fileType==='open_po'?r.po_item:r.pr_item);
      const lineKey=doc&&item?`${code}|${doc}|${item}|${qty}`:null;
      if(lineKey&&seenLines.has(lineKey))continue;
      if(lineKey)seenLines.add(lineKey);
      const current=byMaterial.get(code)||{material_code:code,quantity:0,vendor:null,documents:new Set(),line_count:0,source_rows:[]};
      current.quantity+=qty;current.line_count++;
      if(r.vendor)current.vendor=r.vendor;
      if(doc)current.documents.add(doc);
      if(current.source_rows.length<50)current.source_rows.push({sheet:r.source_sheet,row:r.source_row,document:doc||null,item:item||null,quantity:qty});
      byMaterial.set(code,current);
    }
    return [...byMaterial.values()].map(x=>({
      material_code:x.material_code,
      store_qty:null,
      pr_qty:fileType==='open_pr'?x.quantity:null,
      po_qty:fileType==='open_po'?x.quantity:null,
      vendor:fileType==='open_po'?x.vendor:null,
      document_number:null,
      document_item:null,
      event_date:null,
      expected_date:null,
      metadata:{aggregated:true,line_count:x.line_count,documents:[...x.documents].slice(0,50),source_rows:x.source_rows}
    }));
  }
  return rows.map(r=>({
    ...
    document_item:r.po_item||r.pr_item||r.document_item||null,
