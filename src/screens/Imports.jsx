...
export default function Imports({filters,options,reload,setNotice,refreshToken=0}){
  ...
  return <>
    ...
    <section className="importContext simpleContextCard"><div><strong>Where does this file belong?</strong><span>Set context once before upload.</span></div><div className="importContextFields"><label>Department<select value={departmentCode} onChange={e=>{setDepartmentCode(e.target.value);setArea('')}}>{(options.departments||[]).map(d=><option value={d.department_code} key={d.department_code}>{d.plant_code} · {d.department_name}</option>)}</select></label><label>Equipment<select value={area} onChange={e=>setArea(e.target.value)}><option value="">Not required / mixed</option>{areas.map(a=><option value={a} key={a}>{a}</option>)}</select></label><label>Discipline<select value={discipline} onChange={e=>setDiscipline(e.target.value)}>{disciplines.map(d=><option key={d||'blank'} value={d}>{d||'Read from Excel / blank'}</option>)}</select></label></div><div className="contextLine">{selectedDepartment?`${selectedDepartment.plant_code} · ${selectedDepartment.department_name}`:departmentCode||'No department'}{area?` → ${area}`:''}{discipline?` · ${discipline}`:''}</div></section>
