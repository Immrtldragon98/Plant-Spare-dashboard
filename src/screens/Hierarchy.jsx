...
export default function Hierarchy({rows,departments,reload,setNotice}){
  ...
  return <>
    <div className="pageTitle"><div><h1>SAP Hierarchy</h1><p>Plant → Department → Sub-department Code → Sub-department → Equipment → Equipment Code → Sub-equipment → Sub-equipment Code.</p></div><button onClick={()=>setEdit({department_code:departments?.[0]?.department_code||''})}>+ Add Location</button></div>
    <div className="hierarchyBanner"><strong>Clean hierarchy view</strong><span> Duplicate naming variants are collapsed. SAP department data remains the source of truth.</span></div>
    <div className="tableWrap"><table><thead><tr><th>Plant</th><th>Department</th><th>Sub-department Code</th><th>Sub-department</th><th>Equipment</th><th>Equipment Code</th><th>Sub-equipment</th><th>Sub-equipment Code</th><th>Status</th><th></th></tr></thead><tbody>{rows.map(r=><tr key={r.id}><td className="code"><Blank value={r.plant_code||'3102'}/></td><td>{parentDepartmentName(r)}</td><td><Blank value={r.department_name}/></td><td className="code"><Blank value={r.department_code}/></td><td><Blank value={r.area_name||r.equipment_name}/></td><td className="code"><Blank value={r.equipment_code}/></td><td><Blank value={r.sub_equipment_name}/>{Number(r.duplicate_count)>1&&<small className="muted block">{r.duplicate_count} old rows collapsed</small>}</td><td className="code"><Blank value={r.sub_equipment_code}/></td><td><span className={r.mapping_status==='Mapped'?'mapped':'unmapped'}>{r.mapping_status||'Needs mapping'}</span>{Number(r.active_usages)>0&&<small className="muted block">{r.active_usages} active spare usage(s)</small>}</td><td><button className="link" onClick={()=>setEdit(r)}>Edit</button></td></tr>)}</tbody></table></div>
...
function HierarchyModal({item,departments,onClose,onSaved,setNotice}){
  ...
  return <div className="modal"><form onSubmit={async e=>{e.preventDefault();try{const equipment=String(x.area_name||x.equipment_name||'').trim();const payload={...x,area_name:equipment,area_code:x.department_code||null,equipment_name:equipment,sap_location_code:null};await request(item.id?`/hierarchy/${item.id}`:'/hierarchy',{method:item.id?'PUT':'POST',body:JSON.stringify(payload)});onSaved()}catch(e){setNotice(e.message)}}}>
    ...
    <div className="formGrid">
      ...
      <label>Sub-department Code<select value={x.department_code||''} onChange={e=>{const code=e.target.value;const d=(departments||[]).find(v=>v.department_code===code);setX({...x,department_code:code,department_name:d?.department_name||x.department_name})}}>{(departments||[]).map(d=><option value={d.department_code} key={d.department_code}>{d.department_name}</option>)}</select></label>
      <label>Sub-department<input value={selectedDepartment.department_code||x.department_code||''} readOnly/></label>
      {field('area_name','Equipment','WRM / ICM / PFA / Utility')}
