...
export default function Hierarchy({rows,departments,reload,setNotice}){
  ...
  return <>
    <div className="pageTitle"><div><h1>SAP Hierarchy</h1><p>Plant → Department → Sub-department → Equipment → Equipment Code → Sub-equipment → Sub-equipment Code.</p></div><button onClick={()=>setEdit({department_code:departments?.[0]?.department_code||''})}>+ Add Location</button></div>
