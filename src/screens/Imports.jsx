...
export default function Imports({history,filters,options,reload,setNotice}){
  const[departmentCode,setDepartmentCode]=useState(filters.department_code||''),[area,setArea]=useState(filters.equipment||filters.area||''),[areas,setAreas]=useState(options.equipment||options.areas||[]),[uploadType,setUploadType]=useState('open_po'),[undoing,setUndoing]=useState(null),[analyzedFile,setAnalyzedFile]=useState(null);
  ...
  return <>
    <div className="pageTitle"><div><h1>AI Import & SAP Updates</h1><p>Use the AI Inbox first. Advanced typed imports remain available as a fallback while the universal pipeline matures.</p></div></div>
    <div className="importScope"><label>Sub-department Code<select value={departmentCode} onChange={e=>{setDepartmentCode(e.target.value);setArea('')}}>{(options.departments||[]).map(d=><option value={d.department_code} key={d.department_code}>{d.department_code} — {d.department_name}</option>)}</select></label><label>Equipment<select value={area} onChange={e=>setArea(e.target.value)}><option value="">Select equipment</option>{areas.map(a=><option value={a} key={a}>{a}</option>)}</select></label><div className="scopeSummary"><strong>{selectedDepartment?.department_code||departmentCode}</strong><span>→</span><strong>{area||'Select equipment'}</strong></div></div>
    <UniversalImportInbox departmentCode={departmentCode} equipment={area} discipline={filters.discipline||''} reload={reload} setNotice={setNotice}/>
    ...
    <details className="advancedImports"><summary>Advanced / legacy import tools</summary>
      ...
      <div className="importGrid">
        <div id="master-import"><ImportBox title="Full Spare Master" text="Creates or updates by Material Code. Re-uploading the same master is an update: Preview shows New / Changed / Unchanged before Confirm." preview="/import/master/preview" confirm="/import/master/confirm" departmentCode={departmentCode} area={area} requireArea showDiscipline initialFile={analyzedFile} reload={reload} setNotice={setNotice}/></div>
        <div id="typed-import"><div className="typePicker"><strong>SAP / Procurement Excel Type</strong><div>{Object.entries(types).map(([k,v])=><button type="button" key={k} className={uploadType===k?'active secondary':'secondary'} onClick={()=>setUploadType(k)}>{v.label}</button>)}</div></div>{types[uploadType].planning?<ImportBox key="pr_planning" title="PR Planning / Criticality Screen" text={types[uploadType].text} preview="/pr-planning/preview" confirm={null} departmentCode={departmentCode} area={area} initialFile={analyzedFile} reload={reload} setNotice={setNotice}/>:types[uploadType].transaction?<div className="importBox"><h2>{types[uploadType].label} Import</h2><p>{types[uploadType].text}</p><div className="importDestination"><span>Destination</span><strong>{departmentCode}{area?` → ${area}`:''}</strong></div><div className="fileHelp"><strong>AI mapping ready</strong><span>Use Analyze Excel above to map the file. Historical RGP/NRGP transaction storage remains guarded until dedicated storage is enabled.</span></div></div>:<ImportBox key={uploadType} title={`${types[uploadType].label} Update`} text={types[uploadType].text} preview="/import/sap/preview" confirm="/import/sap/confirm" uploadType={uploadType} departmentCode={departmentCode} initialFile={analyzedFile} reload={reload} setNotice={setNotice}/>}</div>
      </div>
    </details>
    <div className="fileHelp"><strong>Validation stays deterministic</strong><span>LLM understands the workbook. The backend still validates Material Code, numeric quantities, duplicate identity and allowed update fields before saving.</span></div>
