...
export default function ImportBox({title,text,preview,confirm,departmentCode,area,showDiscipline=false,reload,setNotice}){
  ...
  return <div className="importBox">
    ...
    <div><button disabled={!file||busy} onClick={()=>send(preview)}>Preview</button>{data&&<button disabled={busy} onClick={()=>send(confirm)}>Confirm Import</button>}</div>
