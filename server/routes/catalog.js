...
r.get('/options',auth,async(req,res)=>{
  ...
  if(area){const vars=areaVariants(area);p.push(vars);w.push(`(l.equipment_name=ANY($${p.length}) OR l.area_name=ANY($${p.length}))`)}
  ...
  const sub_departments=[...new Set(rows.map(x=>x.area_name).filter(Boolean))].sort();
  const equipmentHierarchy=[...new Map(rows.filter(x=>x.equipment_name).map(x=>[x.equipment_name,{name:x.equipment_name,code:x.equipment_code||null}])).values()].sort((a,b)=>a.name.localeCompare(b.name));
  ...
  res.json({departments,sub_departments,areas:equipmentHierarchy.map(x=>x.name),equipment:equipmentHierarchy.map(x=>x.name),equipment_hierarchy:equipmentHierarchy,sub_equipment:uniq('sub_equipment_name'),vendors,disciplines});
});
export default r;
