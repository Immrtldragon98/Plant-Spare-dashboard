...
export default function MaterialModal({ material, departments, onClose, onSaved, setNotice }) {
  ...
  return <div className="modal"><form onSubmit={async e=>{e.preventDefault();try{await request(m.usage_id?`/materials/${m.usage_id}`:'/materials',{method:m.usage_id?'PUT':'POST',body:JSON.stringify(m)});onSaved()}catch(e){setNotice(e.message)}}}>
    <div className="modalHead"><div><h2>{m.usage_id?'Edit Spare':'Add Spare'}</h2><p className="muted">Store, PR, PO and Vendor can be changed here manually. Use SAP Status Update for bulk changes.</p></div><button type="button" className="ghost" onClick={onClose}>✕</button></div>
    <div className="formGrid">
      {field('material_code','Material Code')}{field('spare_name','Spare Name')}{field('description','Description')}{field('part_number','Part Number')}
      <label>Department<select value={m.department_code||firstDept} onChange={e=>setM({...m,department_code:e.target.value,area:''})}>{(departments||[]).map(d=><option value={d.department_code} key={d.department_code}>{d.department_code} — {d.department_name}</option>)}</select></label>
      {field('area','Area')}{field('equipment','Equipment')}{field('sub_equipment','Sub-equipment')}
      <label>Discipline<select value={m.discipline||''} onChange={e=>setM({...m,discipline:e.target.value})}><option value="">Select discipline</option>{disciplines.map(d=><option key={d}>{d}</option>)}</select></label>
      {field('sap_location_code','SAP Functional Location')}
