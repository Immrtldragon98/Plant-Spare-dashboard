...
import {listKnowledgeDocuments,saveKnowledgeDocument,searchKnowledge} from '../services/knowledge.js';
...
r.post('/knowledge/upload',auth,allow('planner','admin'),upload.single('file'),async(req,res)=>{
  ...
  res.json({ok:true,...out,message:'Document indexed for RAG. Original binary is not archived yet; R2/S3 object storage can be connected later without changing this index.'});
});
