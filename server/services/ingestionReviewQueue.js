...
export async function reviewQueueStatus(){
  return {enabled:await canonicalStagingEnabled()};
}
...
export async function listReviewQueue({status='reviewed',page=1,page_size=25}={}){
  ...
  const allowed=new Set(['received','reviewed','committed','rejected','failed','all']);
  const s=allowed.has(clean(status))?clean(status):'reviewed';
...
export async function decideIngestionBatch({batchId,decision,note='',user}){
  ...
  if(!['stock','open_pr','open_po'].includes(staged.fileType))throw new Error(`${staged.fileType||'Unknown'} batches are not yet supported for human-approved canonical commit`);
  const deterministic=deterministicTransactionReview({fileType:staged.fileType,rows:staged.rows});
  if(!deterministic.writeAllowed)throw new Error(`Approval blocked by deterministic validation: ${deterministic.summary}`);
  const human=await saveHumanDecision(batch.id,'approve',note,user.id);
  try{
    const result=await ingestPlantSnapshots({type:staged.fileType,rows:snapshotRowsFromCanonical(staged.fileType,staged.rows),source:`human-approved:${batch.source_name}`,principal:user.name||user.email||'planner',userId:user.id});
    await markRawBatch(batch.id,'committed',result.batch_id);
    return {ok:true,committed:true,batch_id:batch.id,status:'committed',human_review:human,deterministic,canonical_write:result};
  }catch(error){
    await q(`UPDATE ingestion_human_reviews SET note=COALESCE(note,'') || $1 WHERE batch_id=$2`,[`\nCommit failed: ${String(error.message||error).slice(0,500)}`,batch.id]);
    throw error;
  }
}
