...
export async function analyzeImport(buffer){
  ...
  const prompt=`You analyze industrial SAP/spare Excel workbooks. Return JSON only with {fileType,confidence,mappings,sheetMappings,warnings}. Detect fileType from master, stock, open_pr, open_po, rgp, nrgp, pr_planning. IMPORTANT: each sheet can have a different schema, so return sheetMappings keyed by exact sheet name. Use headers AND sample cell values. Plant conventions: ITEM often means Spare Name only in master-style sheets; numeric values like 10,20,30 under Purchasing Document Item are PO item numbers and MUST map to po_item, never spare_name. MANUFACTURER is Vendor/source; Part name/sparename is Spare Name; TIQ and INST QUANTITY are Required Qty. Assembly Name is hierarchy context. If Short Text + Description + Assembly Name coexist, Description is Spare Name and Short Text is Description. If DESCRIPTION + Short DESCRIPTION + INST QUANTITY coexist, DESCRIPTION is Spare Name and Short DESCRIPTION is Description. PR Quantity is planned_pr_qty; Open PR is pr_qty; Open PO / Still to be delivered is po_qty; Unrestricted is store_qty. For open_po map only procurement-relevant fields and do not infer pr_qty from unrelated quantity columns. For open_pr do not infer po_qty. If a descriptive field cannot be identified confidently, leave it unmapped/blank rather than guessing. Material Code must map only when values match 3 uppercase letters + 12 digits. Never invent data. Allowed keys: ${Object.keys(canonical).join(', ')}. Workbook: ${JSON.stringify(summary)}`;
  try{
