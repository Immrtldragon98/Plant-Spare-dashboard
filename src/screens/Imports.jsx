...
const types={
  ...
  open_pr:{label:'Open PR',text:'Updates only PR quantity by Material Code. PR document numbers are never treated as quantities.'},
  open_po:{label:'Open PO',text:'Updates PO quantity using open / still-to-be-delivered quantity. Vendor is updated when supplied.'}
};
...
export default function Imports({history,filters,options,reload,setNotice}){
  ...
  return <>
    <div className="pageTitle"><div><h1>Excel & SAP Updates</h1><p>Choose destination and file type first. Preview is mandatory before Confirm.</p></div></div>
    <div className="importScope">
      <label>Department / SAP Code<select value={departmentCode} onChange={e=>{setDepartmentCode(e.target.value);setArea('')}}>{(options.departments||[]).map(d=><option value={d.department_code} key={d.department_code}>{d.department_code} — {d.department_name}</option>)}</select></label>
      <label>Area / Sub-area<select value={area} onChange={e=>setArea(e.target.value)}><option value="">Select area</option>{areas.map(a=><option value={a} key={a}>{a}</option>)}</select></label>
    ...
    <div className="importGrid">
      ...
      <div>
        <div className="typePicker"><strong>SAP / Procurement Excel Type</strong><div>{Object.entries(types).map(([k,v])=><button type="button" key={k} className={uploadType===k?'active secondary':'secondary'} onClick={()=>setUploadType(k)}>{v.label}</button>)}</div></div>
        <ImportBox key={uploadType} title={`${types[uploadType].label} Update`} text={types[uploadType].text} preview="/import/sap/preview" confirm="/import/sap/confirm" uploadType={uploadType} departmentCode={departmentCode} reload={reload} setNotice={setNotice}/>
      </div>
    </div>

    <div className="fileHelp"><strong>Material Code rule</strong><span>Exactly 3 uppercase letters + 12 digits, e.g. MMT311715050461. Files with password protection must first be saved as an unprotected .xlsx.</span></div>
