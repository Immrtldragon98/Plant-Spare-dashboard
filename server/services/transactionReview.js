...
export function deterministicTransactionReview(parsed){
  ...
  const qField=quantityField(type),seen=new Map();let blocking=0,warnings=0;
  ...
  rows.forEach((row,index)=>{
    ...
    if(code){const fingerprint=qField?String(n(row[qField])):JSON.stringify([row.spare_name,row.part_number,row.required_qty]);if(seen.has(code)&&seen.get(code)!==fingerprint){findings.push({severity:'error',code:'CONFLICTING_DUPLICATE',row:index+1,material_code:code,message:`Same Material Code appears with conflicting ${qField||'master'} values`});blocking++}else if(seen.has(code)){findings.push({severity:'warning',code:'DUPLICATE_ROW',row:index+1,material_code:code,message:'Duplicate Material Code row carries the same mapped value'});warnings++}else seen.set(code,fingerprint)}
  });
...
export async function llmTransactionReview(parsed,deterministic){
  ...
  const rows=(parsed.rows||[]).slice(0,60).map(r=>({material_code:r.material_code,spare_name:r.spare_name,description:r.description,store_qty:r.store_qty,pr_qty:r.pr_qty,po_qty:r.po_qty,required_qty:r.required_qty,vendor:r.vendor,source_sheet:r.source_sheet,source_row:r.source_row}));
  const prompt=`Review an industrial plant Excel ingestion mapping. You are a semantic reviewer only. Do NOT approve invalid Material Codes, negative quantities, or conflicting duplicates; deterministic validation is authoritative. Check whether mapped business meaning looks plausible for file type ${parsed.fileType}. Distinguish Store, Open PR and Open PO. Never invent missing data. Return only JSON {"decision":"accept|warn|reject","confidence":0.0,"summary":"...","findings":[{"severity":"warning|error","code":"...","message":"...","material_code":"optional"}]}. Mapping/source=${parsed.source}; confidence=${parsed.confidence}; deterministic=${JSON.stringify({decision:deterministic.decision,blocking:deterministic.blocking,warnings:deterministic.warnings,findings:deterministic.findings.slice(0,20)})}; sampleRows=${JSON.stringify(rows)}`;
  try{
