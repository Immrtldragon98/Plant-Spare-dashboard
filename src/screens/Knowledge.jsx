...
const types=['Manual','OEM Catalogue','Datasheet','Engineering Drawing','FMEA','RCA / RCFA','Repair Report','Other'];
...
export default function Knowledge({filters,options={},canEdit,setNotice}){
  ...
  const[overview,setOverview]=useState(null),[loadingOverview,setLoadingOverview]=useState(false),[file,setFile]=useState(null),[busy,setBusy]=useState(false),[query,setQuery]=useState(''),[hits,setHits]=useState([]),[searching,setSearching]=useState(false),[plannerAnswer,setPlannerAnswer]=useState(null),[proposals,setProposals]=useState([]);
  ...
  const loadProposals=async()=>{if(!canEdit)return;try{const x=await request('/knowledge/proposals?status=pending&limit=100');setProposals(x.rows||[])}catch(e){setNotice(e.message)}};
  useEffect(()=>{loadProposals()},[canEdit]);
  const upload=async()=>{if(!file||busy)return;setBusy(true);try{const fd=new FormData();fd.append('file',file);fd.append('department_code',filters.department_code||'');fd.append('equipment',equipment);fd.append('sub_equipment',subEquipment);fd.append('discipline',discipline);fd.append('title',form.title||file.name.replace(/\.[^.]+$/,''));fd.append('document_type',form.document_type);fd.append('material_code',form.material_code||'');const x=await request('/knowledge/upload',{method:'POST',body:fd});setNotice(`${x.title} added. ${x.proposals||0} extracted facts await planner review.`);setFile(null);setForm({title:'',document_type:'Manual',material_code:''});await Promise.all([loadOverview(),loadProposals()])}catch(e){setNotice(e.message)}finally{setBusy(false)}};
  const search=async(searchText=query)=>{const value=text(searchText);if(!value)return;setQuery(value);setSearching(true);setPlannerAnswer(null);try{const x=await request('/knowledge/ask',{method:'POST',body:JSON.stringify({question:value,context:{department_code:filters.department_code,equipment,sub_equipment:subEquipment,discipline},limit:8})});setPlannerAnswer(x);setHits((x.sources||[]).map(s=>({document_id:s.document_id,chunk_index:s.chunk-1,file_name:s.title,text:s.excerpt,metadata:{title:s.title,document_type:s.document_type,equipment:s.equipment,sub_equipment:s.sub_equipment,material_code:s.material_code}})))}catch(e){setNotice(e.message)}finally{setSearching(false)}};
  const reviewProposal=async(id,status)=>{try{await request(`/knowledge/proposals/${id}/review`,{method:'POST',body:JSON.stringify({status})});setProposals(rows=>rows.filter(x=>x.id!==id));setNotice(`Extracted fact ${status}.`)}catch(e){setNotice(e.message)}};
  ...
  return <>
    ...
    <section className="knowledgeSearch simpleKnowledgeSearch"><div className="sectionHead"><div><h2>Ask Plant Knowledge</h2><p>Search the built-in maintenance guide and your indexed drawings, manuals, OEM catalogues, repair reports and RCA records.</p></div></div><div className="knowledgeSearchBar"><input value={query} onChange={e=>setQuery(e.target.value)} onKeyDown={e=>{if(e.key==='Enter')search()}} placeholder="Example: bearing failure checks or motor spare planning"/><button disabled={!query.trim()||searching} onClick={()=>search()}>{searching?'Searching…':'Search'}</button></div>{plannerAnswer&&<article className="plannerAnswer"><div><span className="aiBadge">{plannerAnswer.mode==='llm-grounded'?'GROUNDED AI':'EVIDENCE MODE'}</span><strong>Planner answer</strong></div><p className="plannerAnswerText">{plannerAnswer.answer}</p><small>{plannerAnswer.warning}</small></article>}{query&&guideHits.length===0&&hits.length===0&&!searching&&<p className="muted">No matching plant guide or uploaded document found.</p>}{(guideHits.length>0||hits.length>0)&&<div className="knowledgeHits">{guideHits.map(t=><article key={t.id}><strong>{t.title}</strong><p>{t.purpose} Inspect: {t.inspect.join('; ')}. Plan: {t.plan.join('; ')}.</p><small>Built-in {t.discipline} maintenance guide</small></article>)}{hits.map((h,i)=><article key={`${h.document_id}-${h.chunk_index}-${i}`}><strong>{h.metadata?.title||h.file_name}</strong><p>{h.text}</p><small>{[h.metadata?.document_type,h.metadata?.equipment,h.metadata?.sub_equipment].filter(Boolean).join(' · ')}</small></article>)}</div>}</section>
    ...
    {canEdit&&<section><div className="sectionHead"><div><h2>Planner approval queue</h2><p>AI and rule-based extraction never changes plant master data automatically. Approve only facts confirmed by the source document.</p></div><span className="statusPill warnPill">{proposals.length} pending</span></div>{proposals.length?<div className="proposalGrid">{proposals.slice(0,30).map(p=><article key={p.id}><div><span>{p.fact_type.replaceAll('_',' ')}</span><strong>{p.title||p.file_name}</strong></div><p>{p.source_excerpt}</p><small>{[p.equipment,p.sub_equipment,Math.round(Number(p.confidence)*100)+'% confidence'].filter(Boolean).join(' · ')}</small><div><button onClick={()=>reviewProposal(p.id,'approved')}>Approve</button><button className="secondary" onClick={()=>reviewProposal(p.id,'rejected')}>Reject</button></div></article>)}</div>:<div className="emptyState"><p>No extracted facts are waiting for review.</p></div>}</section>}
