...
export default function Hierarchy({rows,departments,reload,setNotice}){
  ...
  return <>
    <div className="pageTitle"><div><h1>SAP Hierarchy</h1><p>Plant → Department → Area → Equipment → Equipment Code → Sub-equipment → Sub-equipment Code. Duplicate naming variants are collapsed automatically.</p></div><button onClick={()=>setEdit({department_code:departments?.[0]?.department_code||''})}>+ Add Location</button></div>
    <div className="hierarchyBanner"><strong>Clean hierarchy view</strong><span> Old area-only placeholders are hidden. Main Shear / Mainshear-style duplicates are collapsed instead of shown as separate rows.</span></div>
