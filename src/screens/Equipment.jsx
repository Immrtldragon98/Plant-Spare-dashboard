...
export default function Equipment({materials,options,filters,setFilters,setTab}){
  const groups=Object.values(materials.reduce((a,m)=>{
    const child=m.sub_equipment||cleanParent(m.equipment)||'(Equipment level)';
    const k=child||'(Equipment level)';
  ...
  const openGroup=g=>{
    setFilters(f=>({...f,equipment:g.equipment||f.equipment||'',sub_equipment:g.subEquipment||''}));
    setTab('Spares');
  };
  return <>
    <div className="pageTitle"><div><h1>Sub-equipment</h1><p>3102 → {filters.department_code||'3102_CH2'} → {filters.equipment||'All equipment'} → sub-equipment. Equipment choices come from SAP Hierarchy.</p></div></div>
