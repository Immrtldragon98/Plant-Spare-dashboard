...
export default function Imports({history,filters,options,reload,setNotice}){
  ...
  const useMapping=({file,type})=>{
    if(['stock','open_pr','open_po'].includes(type)){setUploadType(type);setAnalyzedFile(file);setTimeout(()=>document.getElementById('typed-import')?.scrollIntoView({behavior:'smooth',block:'start'}),50)}
    ...
    else setNotice(`${String(type).toUpperCase()} was identified. Its transaction importer is being added under Procurement; use the mapping review for now.`)
  };
  ...
  return <>
    ...
    <div className="importGrid">
      ...
      <div id="typed-import"><div className="typePicker"><strong>SAP / Procurement Excel Type</strong><div>{Object.entries(types).map(([k,v])=><button type="button" key={k} className={uploadType===k?'active secondary':'secondary'} onClick={()=>setUploadType(k)}>{v.label}</button>)}</div></div><ImportBox key={uploadType} title={`${types[uploadType].label} Update`} text={types[uploadType].text} preview="/import/sap/preview" confirm="/import/sap/confirm" uploadType={uploadType} departmentCode={departmentCode} initialFile={analyzedFile} reload={reload} setNotice={setNotice}/></div>
    </div>
