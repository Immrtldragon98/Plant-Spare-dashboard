...
export default function ImportBox({title,text,preview,confirm,departmentCode,area,showDiscipline=false,requireArea=false,reload,setNotice}){
  ...
  const send=async(url)=>{
    ...
    try{
      ...
      if(url===confirm){
        const corrected=x.correctedMaterialCodes?`, ${x.correctedMaterialCodes} material codes corrected`:'';
        setNotice(`${title}: ${x.updated||0} updated, ${x.added||0} added, ${x.skipped||0} skipped${corrected}`);
        await reload();
      }
  ...
  return <div className="importBox">
    ...
    {data&&<div className="preview">
      <strong>{data.totalRows??data.total??0} rows found</strong>
      {Number.isFinite(data.exactMatches)&&<span>✓ Exact Material Code matches: {data.exactMatches}</span>}
      {data.safeCorrections?.length>0&&<span>↔ Safe Material Code corrections: {data.safeCorrections.length}</span>}
      {data.missingMaterialCodes?.length>0&&<span>⚠ Material Codes not found in dashboard: {data.missingMaterialCodes.length}</span>}
      {data.disciplineCounts&&<span>Discipline: {Object.entries(data.disciplineCounts).map(([k,v])=>`${k} ${v}`).join(' · ')}</span>}
      {data.unmappedLocations?.length>0&&<span>⚠ {data.unmappedLocations.length} locations need SAP mapping</span>}
      {data.outsideDepartment?.length>0&&<span>⚠ {data.outsideDepartment.length} rows outside selected department</span>}
      {data.issues?.length>0&&<span>⚠ {data.issues.length} file/header issues</span>}
      <small>{data.message}</small>
      {data.safeCorrections?.slice(0,5).map((x,i)=><small key={i}>Fix: {x.current_material_code||'(bad code)'} → {x.material_code} ({x.reason})</small>)}
      {data.missingMaterialCodes?.slice(0,8).length>0&&<small>Missing: {data.missingMaterialCodes.slice(0,8).join(', ')}{data.missingMaterialCodes.length>8?' …':''}</small>}
    </div>}
  </div>;
}
