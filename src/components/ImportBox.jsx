...
export default function ImportBox({title,text,preview,confirm,departmentCode,area,showDiscipline=false,requireArea=false,uploadType='',initialFile=null,reload,setNotice}){
  ...
  const send=async(url)=>{
    ...
    try{
      ...
      if(url===confirm){const corrected=x.correctedMaterialCodes?`, ${x.correctedMaterialCodes} material codes corrected`:'';const unchanged=Number.isFinite(x.unchanged)?`, ${x.unchanged} unchanged`:'';setNotice(`${title}: ${x.updated||0} updated, ${x.added||0} added${unchanged}, ${x.skipped||0} skipped${corrected}`);await reload()}
  ...
  return <div className="importBox">
    ...
    {data&&<div className="preview"><strong>{data.totalRows??data.total??0} rows found</strong>{Number.isFinite(data.newRows)&&<span>＋ New: {data.newRows}</span>}{Number.isFinite(data.changedRows)&&<span>↻ Changed: {data.changedRows}</span>}{Number.isFinite(data.unchangedRows)&&<span>✓ Unchanged: {data.unchangedRows}</span>}{Number.isFinite(data.noCodeRows)&&data.noCodeRows>0&&<span>⚠ No Material Code: {data.noCodeRows}</span>}{data.uploadTypeLabel&&<span>Type: {data.uploadTypeLabel}</span>}{Number.isFinite(data.exactMatches)&&<span>✓ Exact Material Code matches: {data.exactMatches}</span>}{data.safeCorrections?.length>0&&<span>↔ Safe Material Code corrections: {data.safeCorrections.length}</span>}{data.missingMaterialCodes?.length>0&&<span>⚠ Material Codes not found: {data.missingMaterialCodes.length}</span>}{data.disciplineCounts&&<span>Discipline: {Object.entries(data.disciplineCounts).map(([k,v])=>`${k} ${v}`).join(' · ')}</span>}{data.unmappedLocations?.length>0&&<span>⚠ {data.unmappedLocations.length} locations need SAP mapping</span>}{data.issues?.length>0&&<span>⚠ {data.issues.length} file/header issues</span>}<small>{data.message}</small>{data.classifications?.slice(0,12).map((x,i)=><small key={`c${i}`}>{x.status}: {x.material_code||x.spare_name||'No-code row'}{x.sheet?` · ${x.sheet}`:''}</small>)}{data.safeCorrections?.slice(0,5).map((x,i)=><small key={i}>Fix: {x.current_material_code||'(bad code)'} → {x.material_code} ({x.reason})</small>)}{data.missingMaterialCodes?.slice(0,8).length>0&&<small>Missing: {data.missingMaterialCodes.slice(0,8).join(', ')}{data.missingMaterialCodes.length>8?' …':''}</small>}</div>}
  </div>;
}
