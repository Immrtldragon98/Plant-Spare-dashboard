...
export default function Spares(p){
  ...
  return <>
    <div className="pageTitle"><div><h1>Spare Materials</h1><p>Current Store, Open PR and Open PO quantities are shown directly against each Material Code.</p></div><div>{p.canEdit&&<button onClick={()=>p.setEditing(true)}>+ Add Spare</button>} <button className="secondary" onClick={download}>Export Excel</button></div></div>
    <Filters {...p}/>
