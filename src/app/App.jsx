...
export default function App(){
 ...
 const reload=async()=>{if(!user)return;const qs=new URLSearchParams({...filters,search});const [s,o]=await Promise.all([request('/dashboard?'+qs),request('/options?'+qs)]);setStats(s);setOptions(o);if(tab==='Vendors')setVendors(await request('/vendors?'+qs));if(tab==='Imports'&&user.role!=='viewer')setHistory(await request('/import-history'));if(tab==='Users'&&isAdmin(user.role))setUsers(await request('/users'));if(tab==='SAP Hierarchy'&&isAdmin(user.role))setHierarchy(await request('/hierarchy?'+new URLSearchParams({department_code:filters.department_code})));setRefreshToken(x=>x+1)};
